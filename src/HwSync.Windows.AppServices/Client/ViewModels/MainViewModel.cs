...
namespace HwSync.Windows.AppServices.Client.ViewModels
{
    ...
    {
        System.Windows.Input.ICommand IMainViewModel.ConnectCommand => ConnectCommand;

        System.Windows.Input.ICommand IMainViewModel.StartCommand => StartCommand;

        System.Windows.Input.ICommand IMainViewModel.ResumeCommand => ResumeCommand;

        System.Windows.Input.ICommand IMainViewModel.CopyCommand => CopyCommand;

        System.Windows.Input.ICommand IMainViewModel.UploadCommand => UploadCommand;

        System.Windows.Input.ICommand IMainViewModel.DeleteServerCommand => DeleteServerCommand;

        System.Windows.Input.ICommand IMainViewModel.DeleteClientCommand => DeleteClientCommand;

        System.Windows.Input.ICommand IMainViewModel.CancelCommand => CancelCommand;

        HwSync.Windows.Contract.Client.Diagnostics.ITransferMetrics? IMainViewModel.TransferMetrics => TransferMetrics;

        System.Windows.Input.ICommand IMainViewModel.AutoSyncCommand => AutoSyncCommand;

        private readonly HwSync.Windows.Contract.Client.Services.ISyncDecisionService _decisions;
        private readonly Func<Uri, IHwSyncApiClient> _createClient;
        private readonly IFileSnapshotProvider _snapshotProvider;
        private readonly IComparedFileOperations _fileOperations;
        private readonly ISourceFileReader _fileReader;
        private readonly IConflictFileOperations _conflictFiles;
        private readonly IFolderHistory? _history;
        private readonly IMissingFileSynchronizer _synchronizer;
            ...
            IComparedFileOperations fileOperations, ISourceFileReader fileReader, IMissingFileSynchronizer synchronizer, HwSync.Windows.Contract.Client.Services.ISyncDecisionService decisions, IConflictFileOperations conflictFiles, IFolderHistory? history = null)
        {
            ...
            _history = history;
            ResolveConflictCommand = new RelayCommand<ChangeRow>(ResolveConflict, row =>
                !_busy && _completedComparison is not null && row is { IsConflict: true } && Changes.Contains(row));
            AutoSyncCommand = new AsyncRelayCommand(AutoSyncAsync, CanAutoSync);
        ...
        public IAsyncRelayCommand AutoSyncCommand { get; }
